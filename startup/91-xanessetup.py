...
def xanes_afterscan(scanid, roinum, filename, i0scale, itscale, roi_key, struck = False,detune=None):
    ...
    if struck == True:
        ...
        columnitem = ['energy_energy', 'energy_u_gap_readback', 'energy_bragg', 'energy_c2_x',
                    'sclr_i0', 'sclr_it', roi_key[0], roi_key[1], roi_key[2]]    
    else:
        columnitem = ['energy_energy', 'energy_u_gap_readback', 'energy_bragg', 'energy_c2_x',
    ...
    if struck == True:
        datatable = get_table(h, ['sclr_i0', 'sclr_it', roi_key[0], roi_key[1], roi_key[2]],
                          stream_name='primary')        
#        i0_array = abs(numpy.array(datatable['sclr_i0']) - i0_baseline) * i0scale
#        it_array = abs(numpy.array(datatable['sclr_it']) - it_baseline) * itscale
...
#    if struck == True:
#        usercolumnitem['It'] = numpy.zeros(len(i0_array))
#    else:
...
def xanes(erange = [], estep = [],  
    ...
    #output the datafile
    xanes_afterscan(scaninfo[0], roinum, filename, i0scale, itscale, roi_key,struck=struck)
...
def xfmxanes_xybatch(xylist=[], waittime = None, 
                    samplename = None, filename = None,
                    erange = [], estep = [],  
                    harmonic = None, correct_c2_x=True, delaytime=0.0,             
                    acqtime=None, roinum=1, i0scale = 1e8, itscale = 1e8,
                    ):
