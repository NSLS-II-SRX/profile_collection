...
class SRX_M2_All(SRX_M2):
    ...
    # the following are read-only signals because they shall not be
    # moved indvidually
...
class SRXTomo(Device):
    ...
    # PiezoJena
    finex_top = Cpt(EpicsMotor, ':1{Stg:Tomo-Ax:XFT}Mtr',
                    settle_time=piezo_jena_settle_time)
    finey_top = Cpt(EpicsMotor, ':1{Stg:Tomo-Ax:YFT}Mtr',
                    settle_time=piezo_jena_settle_time)
    finez_top = Cpt(EpicsMotor, ':1{Stg:Tomo-Ax:ZFT}Mtr',
                    settle_time=piezo_jena_settle_time)
